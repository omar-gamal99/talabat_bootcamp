...
for config_path in config_files:
    ...
    if missing_keys:
        raise KeyError(f"Required keys {missing_keys} are missing in the YAML file: {config_path}")

    original_dag_id = config["dag_id"]
    dag_id = f"{original_dag_id}_abdullah"  # Append 'abdullah' to DAG ID
    schedule = config.get("schedule_interval", None)
    tables = config["tables"]

    default_args = {
        "owner": config.get("default_args", {}).get("owner", "DE team"),
        "email": config.get("default_args", {}).get("email", ["data.eng.team@talabat.com"]),
        "start_date": datetime.strptime(config.get("default_args", {}).get("start_date", "2024-05-02"), "%Y-%m-%d"),
